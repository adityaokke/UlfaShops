...
class Image {
	...
	public static function resize($source,$path,$desti, $options = array()) {
		...
		$dataSave = $desti .$cropstring.basename($path);

		if (!is_dir(WWW_ROOT . DS . 'img' . DS . 'cache')) {
			mkdir(WWW_ROOT . DS . 'img' . DS . 'cache');
		}

		if (file_exists($dataSave)) {
			$csize = getimagesize($dataSave);
			$cached = ($csize[0] == $width && $csize[1] == $height); // image is cached
			if (filemtime($dataSave) < filemtime($fullpath)) { // check if up to date
				$cached = false;
			}
		} else {
			$cached = false;
		...
		if ($resize) {
			...
			if ($types[$size[2]] == "jpeg") {
				imagejpeg($temp, $dataSave, $quality);
				
			} 
			else {
				
				call_user_func("image" . $types[$size[2]], $temp, $dataSave);
			}
			imagedestroy($image);
			imagedestroy($temp);
		} elseif (!$cached) {
			...
			imagejpeg($image, $dataSave, $quality);
